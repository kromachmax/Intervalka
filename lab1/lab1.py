...
def main():
    ...
    left = 0
    alpha = right = max(abs(midA[0][0]), abs(midA[0][1]), abs(midA[1][0]), abs(midA[1][1]))
    while right - left > eps:
        middle = (right + left) / 2
        for i in range(len(A)):
            for j in range(len(A[i])):
                A[i][j][0] = midA[i][j] - middle * radA[i][j]
                A[i][j][1] = midA[i][j] + middle * radA[i][j]
